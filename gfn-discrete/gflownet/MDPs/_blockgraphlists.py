...
class GraphLists:
    ...
    def add_edge(self, n1, n2, features):
        ...
        #     f'{n1=}, {n2=}, {len(self)=}'
        assert n1 < len(self) and n2 < len(self), f"n1={n1}, n2={n2}, len(self)={len(self)}"
    ...
    def to_nx(self):
        ...
        for node_idx in self.edges:
            for neighbor_idx in self.edges[node_idx]:
                graph.add_edge(
                    node_idx, neighbor_idx, nn_features=self.edge_features[node_idx][neighbor_idx]
                )
        return graph
...
class BGLeditor:
    def __init__(self, blocks_file):
        ...
        self.block_mols = [Chem.MolFromSmiles(smi) for smi in self.blocks["block_smi"]]
        self.block_natm = np.asarray([b.GetNumAtoms() for b in self.block_mols])
        self.smi_to_natm = {smi: natm for smi, natm in zip(self.block_smi, self.block_natm)}
    ...
    def add_block(self, bgl, block_id, stem_idx=None, new_atom_idx=None):
        ...
        if len(c.blockids) == 1:
            ...
            #     \n{bgl=}\n{c=}\n{block_id=}\n{stem_idx=}\n{new_atom_idx=}'
            assert (
                stem_idx is not None and new_atom_idx is not None
            ), f"If bgl has blocks, adding block must specify either stem_idx \
          or new_atom_idx.\
    ...
    def num_atom_types(self):
        ...
        return len(set(atom.GetAtomicNum() for mol in self.block_mols for atom in mol.GetAtoms()))

    # @functools.cached_property
    def atom_num_to_id_map(self):
        atom_nums = set(atom.GetAtomicNum() for mol in self.block_mols for atom in mol.GetAtoms())
...
def randomwalk(length=8):
    ...
    while bgl.numblocks < length:
        ...
        bgl = editor.add_block(bgl, block_id=block_id, stem_idx=stem_idx, new_atom_idx=0)
