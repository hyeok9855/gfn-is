...
class SeqPAState(BaseState):
    ...
    def regenerate(self, content):
        return SeqPAState(content, is_leaf=True)
...
class SeqPrependAppendMDP(BaseMDP):
    ...
    def __init__(self, args, alphabet=list("0123"), forced_stop_len=8):
        ...
        self.fwd_actions = (
            [SeqPAAction(SeqPAActionType.Stop)]
            + [SeqPAAction(SeqPAActionType.PrependChar, c) for c in self.alphabet]
            + [SeqPAAction(SeqPAActionType.AppendChar, c) for c in self.alphabet]
        )
        self.back_actions = [
            SeqPAAction(SeqPAActionType.UnStop),
...
class SeqPAActor(Actor):
    ...
    def __init__(self, args, mdp):
        ...
        self.char_to_idx = {a: i for (i, a) in enumerate(sorted(self.alphabet))}
    ...
    # Featurization
    @functools.lru_cache(maxsize=None)
    def featurize(self, state: SeqPAState):
        ...
        full_ohe_dim = ohe_dim(self.forced_stop_len)
        embed = torch.zeros(full_ohe_dim, dtype=torch.float, device=self.args.device)
        if len(state.content) > 0:
            contents = torch.tensor(
                [self.char_to_idx[c] for c in state.content], device=self.args.device
            )
            contents_oh = torch.nn.functional.one_hot(contents, num_classes=len(self.alphabet))
            embed[: ohe_dim(len(state.content))] = contents_oh.flatten()
        return embed
