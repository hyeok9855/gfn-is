...
class BaseMDP:
    ...
    def get_children(self, state):
        ...
        """
        return [self.transition_fwd(state, act) for act in self.get_fwd_actions(state)]

    def get_parents(self, state):
        """Return list of children in deterministic order.
        Calls self.transition_back on actions from self.get_back_actions.
        """
        return [self.transition_back(state, act) for act in self.get_back_actions(state)]
