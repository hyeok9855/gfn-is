    ...
    net = nn.Sequential(
        *(
            sum(
                [
                    [nn.Linear(i, o)]
                    + (([nn.BatchNorm1d(o), act] if with_bn else [act]) if n < len(l) - 2 else [])
...
def make_convnet(
    inp_side, kernel_size, num_channels, mlp_hid_dim, mlp_n_layers, mlp_out_dim, act=nn.LeakyReLU()
):
...
# SA
def make_molblock_back_sa_gnn(
    num_block_types, num_edge_types, embed_dim, hid_dim, n_layers, out_node_dim, global_out_dim
):
...
# SSR
def make_molblock_ssr_gnn(num_node_types, num_edge_types, embed_dim, hid_dim, n_layers, out_dim):
    class GNN(torch.nn.Module):
