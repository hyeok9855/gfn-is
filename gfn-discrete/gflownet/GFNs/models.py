...
class TeacherGFN(TeacherGFlowNet):
    """Trajectory balance GFN. Learns forward and backward policy."""
    ...
    def train(self, batch, adv_logreward, round_num):
        return self.train_tb(batch, adv_logreward, round_num)


class GAFN(AugGFlowNet):
    def __init__(self, args, mdp, actor, rnd_target, rnd_predict):
        super().__init__(args, mdp, actor, rnd_target, rnd_predict)
        print("Model: GAFN")
...
class SubTBGFN(BaseTBGFlowNet):
    ...
    def init_subtb(self):
        ...
        tidx = [
            torch.tril_indices(i, i, device=self.args.device)[1] for i in range(self.subtb_max_len)
        ]
        ...
        self.precomp = [
            (
                ...
                torch.cat(
                    [
                        ar[: T - i].repeat_interleave(ar[: T - i] + 1) + ar[T - i + 1 : T + 1].sum()
                        for i in range(T)
                    ]
                ),
            )
    ...
    def batch_loss_sub_trajectory_balance(self, batch):
        ...
        for i in range(len(batch)):
            ...
            total_loss[i] = (
                weight * (F_start - F_end + P_F_sums - P_B_sums).pow(2)
...
def make_model(args, mdp, actor, rnd_target=None, rnd_predict=None):
    """Constructs MaxEnt / TB / Sub GFN."""
    if args.model == "maxent" or "tb" or "teacher":
        model = MaxEntGFN(args, mdp, actor)
    elif args.model == "gafn":
        model = GAFN(args, mdp, actor, rnd_target, rnd_predict)
...
def make_teacher_student_model(args, mdp, mdp_teacher, actor, actor_teacher):
    """Constructs Teacher GFN."""
    model_teacher = TeacherGFN(args, mdp_teacher, actor_teacher)
    model_student = make_model(args, mdp, actor)
