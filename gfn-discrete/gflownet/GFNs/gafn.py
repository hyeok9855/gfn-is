...
class AugGFlowNet:
    ...
    def batch_traj_bwd_logp(self, batch, evaluate=False):
        ...
        if not evaluate:
            state_emb = self.policy_fwd.encoder(back_states)
            ...
            with torch.no_grad():
                predict = self.rnd_predict.encoder(back_states)
                target = self.rnd_target.encoder(back_states)
            ...
            back_logp_chosen = [
                s2lp[p]
                for s2lp, p, flow, int_reward in zip(
                    states_to_logps, fwd_states, state_flow, intrinsic_reward
                )
            ]
            sum_reward = intrinsic_reward.view(-1, seq_len).sum(dim=1)
            back_chain = torch.stack(
                [
                    torch.log(torch.exp(exp.logr) + int_reward)
                    for exp, int_reward in zip(batch, sum_reward)
                ]
            )
        else:
    ...
    def batch_loss_rnd(self, batch):
        ...
        random_pi = self.rnd_target.encoder(fwd_states)
        predict_pi = self.rnd_predict.encoder(fwd_states)
    ...
    def train_tb(self, batch, log=True, online=False):
        ...
        if log:
            if online:
                ...
                wandb.log(
                    {
                        "Online Regular TB loss": batch_loss.item(),
                        "online_loss_step": self.online_loss_step,
                    }
                )
                wandb.log(
                    {"Online RND loss": rnd_loss.item(), "online_loss_step": self.online_loss_step}
                )
            else:
                self.offline_loss_step += 1
                wandb.log(
                    {
                        "Offline Regular TB loss": batch_loss.item(),
                        "offline_loss_step": self.offline_loss_step,
                    }
                )
                wandb.log(
                    {
                        "Offline RND loss": rnd_loss.item(),
                        "offline_loss_step": self.offline_loss_step,
                    }
                )
        return
