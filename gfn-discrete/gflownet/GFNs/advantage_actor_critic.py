...
class A2C:
    ...
    def __init__(self, args, mdp, actor):
        ...
        net = make_mlp([self.actor.ft_dim] + [hid_dim] * n_layers + [1])
        ...
        self.clip_grad_norm_params = [self.policy.parameters(), self.critic.parameters()]

        self.optimizer = torch.optim.Adam(
            [
                {"params": self.policy.parameters(), "lr": args.lr_policy},
    ...
    def batch_loss_advantage_actor_critic(self, batch):
        ...
        G = (
            torch.FloatTensor([exp.r for exp in batch])
            .repeat_interleave(self.mdp.forced_stop_len + 1)
            .to(self.args.device)
        )
        A = G - V
