...
class TeacherGFlowNet:
    ...
    def __init__(self, args, mdp, actor):
        ...
        self.optimizer_fwd = torch.optim.Adam(
            [{"params": self.policy_fwd.parameters(), "lr": 5.0 * args.lr_policy}]
        )
        self.optimizer_logZ = torch.optim.SGD(
            [{"params": self.logZ, "lr": args.lr_z, "momentum": 0.8}]
        )

        self.optimizers = [self.optimizer_fwd, self.optimizer_logZ]
        pass
    ...
    def annealed_temperature(self, round_num):
        init_temp = 0.1
        final_temp = self.args.teacher_beta
        anneal_rate = max(2 * round_num / self.args.num_active_learning_rounds, 1)
    ...
    def batch_traj_bwd_logp(self, batch, adv_logreward, round_num):
        ...
        temp = self.annealed_temperature(round_num)
        back_chain_adv = torch.stack([adv_logr for adv_logr in adv_logreward])
        back_chain_reward = torch.stack([exp.logr for exp in batch])

        # back_chain = self.args.reward_augment * (self.args.adv_alpha * back_chain_adv + (1-self.args.adv_alpha) * back_chain_reward)
        back_chain = back_chain_adv + self.args.alpha * back_chain_reward
    ...
    def batch_loss_trajectory_balance(self, batch, adv_logreward, round_num):
    ...
    def train_tb(self, batch, adv_logreward, round_num, log=True):
        ...
        """
        batch_loss = self.batch_loss_trajectory_balance(batch, adv_logreward, round_num)
        ...
        if log:
            ...
            print(f"Teacher training:", batch_loss)
            wandb.log({"Regular Teacher loss": batch_loss})
