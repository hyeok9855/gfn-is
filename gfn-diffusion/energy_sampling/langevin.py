...
def adjust_ld_step(
    current_ld_step, current_acceptance_rate, target_acceptance_rate=0.574, adjustment_factor=0.01
):
    """
...
def langevin_dynamics(x, log_reward, device, args):
    ...
    for i in range(args.max_iter_ls):
        ...
        if args.ld_schedule:
            ld_step = (
                ...
                if i == 0
                    ...
                    ld_step, acceptance_rate, target_acceptance_rate=args.target_acceptance_rate
                )
            )
        else:
        ...
        log_q_fwd = -(torch.norm(new_x - x - ld_step * r_grad_original, p=2, dim=1) ** 2) / (
            4 * ld_step
        )
