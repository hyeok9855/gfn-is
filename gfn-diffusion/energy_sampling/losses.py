...
def tb_subtb_loss(  # TB + SubTB (chunk)
    ...
    subtb_losses = subtb_chunk_loss_v2(log_pfs.detach(), log_pbs.detach(), log_fs, chunk_size)
...
def get_loss(
    ...
    if loss_type == "tb":
        ...
        if subtb_chunk_size > 0:  # Chunk-based subtb
            losses = subtb_chunk_loss_v2(log_pfs, log_pbs, log_fs, subtb_chunk_size)
        else:
